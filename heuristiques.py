...
class HeuristiqueRecursif():
    ...
    def RechercheRecursive(self, villes_possibles, parcours_recursif, distance_recursive):
        ...
        if len(villes_possibles) == 0:
            distance_recursive_finale = distance_recursive + math.dist(self.villes.dict[parcours_recursif[-1]], self.villes.dict[self.villes.depart])
            if distance_recursive_finale < self.meilleur_parcours_recursif[1]:
                self.meilleur_parcours_recursif = (parcours_recursif[:], distance_recursive_finale)
            return
        for idville in villes_possibles:
